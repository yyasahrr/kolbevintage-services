  ...
  describe("B1: Real Backup -> Restore Smoke Verification", () => {
    ...
    it("seeds rich domain records, backs up, verifies checksum, and restores cleanly", async () => {
      ...
      if (restoreResult.tablesCount !== undefined) {
        expect(restoreResult.tablesCount).toBe(198); // 5.9-C alters the refund engine in place (no new tables)
      }
      ...
      try {
        ...
        `);
        expect(countRes.rows[0].c).toBe(198); // 5.9-C alters the refund engine in place (no new tables)
