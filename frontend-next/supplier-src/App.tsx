...
export function Dashboard({ onNavigate }: { onNavigate: (page: Page) => void }) {
  ...
  return <>
    ...
    <section className="dashboard-grid"><div className="action-center"><SectionHeading eyebrow="صف اقدام تأمین‌کننده" title="کارهایی که منتظر شما هستند" action={<TextButton onClick={() => onNavigate('orders')}>مشاهده همه</TextButton>}>برای حفظ SLA و جریان سفارش، این موارد را به‌ترتیب انجام دهید.</SectionHeading><div className="action-list"><ActionItem count="۵" title="سفارش برای تأیید دارید" note="قدیمی‌ترین سفارش ۲ ساعت تا پایان مهلت دارد." icon={<Truck size={19}/>} tone="warm" action="بررسی سفارشات" onClick={() => onNavigate('orders')} /><ActionItem count="۳" title="RFQ نیازمند قیمت‌گذاری است" note="یک درخواست، موعد تحویل نزدیک دارد." icon={<MessageSquareText size={19}/>} tone="blue" action="باز کردن RFQها" onClick={() => onNavigate('rfqs')} /><ActionItem count="۲" title="محصول به اصلاح نیاز دارد" note="اطلاعات پارچه و گالری تصاویر کامل نیست." icon={<FileCheck2 size={19}/>} tone="rose" action="رفع اصلاحات" onClick={() => onNavigate('products')} /><ActionItem count="۱" title="نمونه منتظر بارگذاری است" note="برای سفارش تولید PO-4827" icon={<Upload size={19}/>} tone="ink" action="بارگذاری نمونه" onClick={() => onNavigate('production')} /></div></div>
      <div className="side-stack"><section className="today-panel"><SectionHeading title="امروز در عملیات" action={<button className="icon-button"><PanelRight size={17}/></button>}/>{products.length ? <><div className="mini-order"><img src={products[0].image} alt={products[0].name}/><div><b>{products[0].name}</b><span>{products[0].sku} · {products[0].series} سری قابل فروش</span></div><Status>{products[0].status}</Status></div><div className="mini-order"><span className="rfq-avatar">ه</span><div><b>RFQ-2048</b><span>۶۰۰ تکه · پیراهن اختصاصی</span></div><Status>نیازمند قیمت‌گذاری</Status></div><div className="mini-order"><span className="production-mark"><FactoryIcon/></span><div><b>PO-4827</b><span>تولید نمونه · ۴ روز تأخیر</span></div><Status>پیگیری فوری</Status></div></> : <div className="mini-order-empty"><b>هنوز محصولی ثبت نشده است</b><span>برای دیده‌شدن در کاتالوگ کلبه، نخستین محصول خود را بسازید.</span><button className="button primary" onClick={() => onNavigate('product-editor')}><Plus size={15}/>افزودن محصول</button></div>}</section><section className="stock-watch"><div><div><p className="eyebrow">پایش موجودی</p><h3>۴ محصول کم‌موجود</h3><span>پایین‌تر از حداقل سری قابل فروش</span></div><Activity size={21}/></div><div className="stock-bars"><StockBar name="کت کتان دو دکمه" value={14}/><StockBar name="دامن پلیسه کمربندی" value={22}/><StockBar name="پیراهن لینن کوتاه" value={30}/></div><TextButton onClick={() => onNavigate('inventory')}>مدیریت موجودی</TextButton></section></div>
    </section>
    <section className="dashboard-bottom"><section className="surface order-preview"><SectionHeading title="آخرین سفارشات آماده" action={<TextButton onClick={() => onNavigate('orders')}>همه سفارشات</TextButton>}/>{orderRows.length ? <div className="compact-table">{orderRows.slice(0, 3).map(row => <div className="compact-row" key={row.id}><b>{row.id}</b><span>{row.customer}</span><span>{row.value}</span><Status>{row.status}</Status><RowMenu/></div>)}</div> : <div className="compact-table"><div className="compact-row empty-row"><b>سفارش آماده‌ای وجود ندارد</b><span>به‌محض ثبت سفارش جدید، اینجا نمایش داده می‌شود.</span></div></div>}</section><section className="surface review-panel"><SectionHeading title="وضعیت تأیید کاتالوگ"/><div className="review-stat"><strong>۳۷</strong><span>محصول فعال در کولبه</span><div className="stacked-line"><i/><i/><i/></div></div><div className="review-notes"><span><i className="green-dot"/>۳۷ تأیید شده</span><span><i className="amber-dot"/>۲ نیازمند اصلاح</span></div><TextButton onClick={() => onNavigate('products')}>مشاهده محصولات</TextButton></section></section>
  </>
}
...
export function Products({ onNavigate, query, setQuery }: { onNavigate: (page: Page) => void; query: string; setQuery: (value: string) => void }) {
  ...
  return <><div className="page-head"><div><PageCrumbs parent="کاتالوگ" current="محصولات"/><h1>محصولات</h1><p>کاتالوگ، تأیید کولبه و قابلیت فروش سری‌های خود را مدیریت کنید.</p></div><button className="button primary" onClick={() => onNavigate('product-editor')}><Plus size={17}/>محصول جدید</button></div><section className="surface table-surface"><div className="tabs">{['همه محصولات', 'فعال', 'در بررسی', 'نیازمند اصلاح', 'پیش‌نویس'].map(item => <button onClick={() => setTab(item)} className={tab === item ? 'active' : ''} key={item}>{item}{item === 'نیازمند اصلاح' ? <em>۲</em> : null}</button>)}</div><div className="table-toolbar"><label className="search-field"><Search size={17}/><input value={query} onChange={event => setQuery(event.target.value)} placeholder="جست‌وجو با نام یا SKU"/></label><div className="tool-actions"><button className="button secondary"><SlidersHorizontal size={16}/>فیلترها</button><button className="button secondary hide-mobile"><CalendarDays size={16}/>به‌روزرسانی اخیر</button><button className="icon-button"><MoreHorizontal/></button></div></div><div className="desktop-table">{filtered.length ? <><div className="table-row table-header"><span>محصول</span><span>وضعیت</span><span>سری قابل فروش</span><span>موجودی فیزیکی</span><span>قیمت عمده</span><span>آخرین تغییر</span><span></span></div>{filtered.map(product => <div className="table-row product-row" key={product.id}><div className="product-cell"><img src={product.image} alt=""/><div><b>{product.name}</b><span>{product.sku} · {product.category}</span></div></div><Status>{product.status}</Status><span className={product.series === 0 ? 'zero-number' : 'number'}>{product.series} سری</span><span className={product.stock < 10 ? 'low-number' : 'number'}>{product.stock} تکه</span><b>{product.price}</b><span className="muted">{product.updated}</span><RowMenu/></div>)}</> : <div className="empty-state"><div className="empty-mark"><Package size={22}/></div><h3>هنوز محصولی ثبت نشده است</h3><p>کاتالوگ شما خالی است. نخستین محصول را بسازید تا در فروشگاه کلبه نمایش داده شود.</p><button className="button primary" onClick={() => onNavigate('product-editor')}><Plus size={16}/>ایجاد اولین محصول</button></div>}</div><div className="mobile-product-list">{filtered.map(product => <article key={product.id}><img src={product.image} alt=""/><div><b>{product.name}</b><span>{product.sku} · {product.series} سری قابل فروش</span><Status>{product.status}</Status></div><RowMenu/></article>)}</div><footer className="table-footer"><span>نمایش {filtered.length} از ۴۳ محصول</span><div><button className="icon-button">‹</button><b>۱</b><button className="icon-button">›</button></div></footer></section></>
}
...
export function Orders() { const [selected, setSelected] = useState(''); return <><div className="page-head"><div><PageCrumbs parent="عملیات" current="سفارشات آماده"/><h1>سفارشات آماده</h1><p>سفارش‌ها را به‌موقع تأیید و برای ارسال آماده کنید.</p></div><button className="button secondary"><FileText size={16}/>خروجی سفارشات</button></div><section className="surface orders-surface"><div className="table-toolbar"><label className="search-field"><Search size={17}/><input placeholder="شناسه سفارش، مشتری یا محصول"/></label><div className="tool-actions"><button className="button secondary"><SlidersHorizontal size={16}/>فیلتر</button><button className="button secondary">نمای ذخیره‌شده <ChevronDown size={15}/></button></div></div>{orderRows.length ? <><div style={{marginBottom:16}}><OrderSLA orderId={selected} slaHours={24} /></div>
<div style={{marginBottom:16}}><ShippingLabel orderId={selected} items={[{ name: 'پیراهن آکسفورد', qty: 30 }]} /></div>
<div className="orders-layout"><div className="orders-list"><div className="order-head-row"><span>سفارش</span><span>محصول و سری</span><span>مبلغ</span><span>مهلت ارسال</span><span>وضعیت</span></div>{orderRows.map(order => <button onClick={() => setSelected(order.id)} className={`order-list-row ${selected === order.id ? 'selected' : ''}`} key={order.id}><div><b>{order.id}</b><span>{order.customer}</span></div><div><b>{order.product}</b><span>{order.pack} · {order.quantity}</span></div><b>{order.value}</b><span>{order.due}</span><Status>{order.status}</Status></button>)}</div><OrderInspector id={selected}/></div></> : <div className="empty-state"><div className="empty-mark"><Truck size={22}/></div><h3>سفارش آماده‌ای در انتظار شما نیست</h3><p>وقتی کلبه سفارش عمده‌ای برای شما ثبت کند، آن را اینجا برای تأیید و ارسال می‌بینید.</p></div>}</section></> }

export function OrderInspector({ id }: { id: string }) { const order = orderRows.find(row => row.id === id); if (!order) return null; return <aside className="order-inspector"><div className="inspector-head"><div><p className="eyebrow">جزئیات سفارش</p><h2>{order.id}</h2></div><RowMenu/></div><Status>{order.status}</Status><div className="inspector-product"><img src={(products.find(product => product.name === order.product) ?? products[0])?.image ?? ''} alt=""/><div><b>{order.product}</b><span>{order.pack} · {order.quantity}</span></div></div><dl><div><dt>تعداد کل</dt><dd>{order.pieces}</dd></div><div><dt>ارزش سفارش</dt><dd>{order.value}</dd></div><div><dt>تاریخ ارسال</dt><dd className="low-number">{order.due}</dd></div></dl><div className="reservation"><div><b>رزرو موجودی</b><span>با تأیید شما اعمال خواهد شد.</span></div><strong>{order.quantity}</strong></div><div className="inspector-actions"><button className="button primary"><Check size={16}/>تأیید سفارش</button><button className="button secondary">گزارش مغایرت</button><button className="link-danger">رد با ذکر دلیل</button></div></aside> }

export function RFQInbox({ onNavigate }: { onNavigate: (page: Page) => void }) { const [selected, setSelected] = useState(''); const current = rfqs.find(x => x.id === selected); return <><div className="page-head"><div><PageCrumbs parent="تولید سفارشی" current="صندوق RFQ"/><h1>درخواست‌های تولید</h1><p>درخواست‌های واجد ظرفیت کارخانه را بررسی و برای آن‌ها پیشنهاد ارسال کنید.</p></div><button className="button secondary"><SlidersHorizontal size={16}/>فیلترهای هوشمند</button></div>{rfqs.length ? <section className="rfq-layout"><div className="surface rfq-list"><div className="list-tabs"><button className="active">همه <em>{rfqs.length}</em></button><button>نیازمند پاسخ <em>۳</em></button><button>پیشنهادها</button></div>{rfqs.map(rfq => <button key={rfq.id} onClick={() => setSelected(rfq.id)} className={`rfq-row ${selected === rfq.id ? 'selected' : ''}`}><span className="rfq-avatar">{rfq.avatar}</span><div><div><b>{rfq.title}</b><Status>{rfq.status}</Status></div><span>{rfq.customer} · {rfq.quantity}</span><small>{rfq.id} · موعد پاسخ: {rfq.deadline}</small></div></button>)}</div>{current ? <RFQDetail rfq={current} onQuote={() => onNavigate('quote')}/> : <aside className="surface rfq-detail"><div className="rfq-detail-head"><div><p className="eyebrow">درخواستی انتخاب نشده است</p><h2>جزئیات RFQ</h2><p>یک درخواست از لیست انتخاب کنید.</p></div></div></aside>}</section> : <div className="surface empty-state"><div className="empty-mark"><MessageSquareText size={22}/></div><h3>درخواست تولید جدیدی وجود ندارد</h3><p>وقتی کلبه درخواست تولید سفارشی برای شما ارسال کند، آن را اینجا می‌بینید.</p></div>}</> }

export function RFQDetail({ rfq, onQuote }: { rfq: typeof rfqs[number]; onQuote: () => void }) { return <section className="surface rfq-detail"><div className="rfq-detail-head"><div><p className="eyebrow">{rfq.id} · درخواست جدید</p><h2>{rfq.title}</h2><p>{rfq.customer}</p></div><Status>{rfq.status}</Status></div><div className="request-banner"><div><p>فرصت برآورد</p><strong>{rfq.quantity}</strong><span>حداقل سفارش درخواست‌شده</span></div><div><p>تاریخ تحویل</p><strong>{rfq.deadline}</strong><span>تحویل در انبار کولبه</span></div><div><p>پارچه پیشنهادی</p><strong>{rfq.fabric}</strong><span>نمونه پارچه پیوست شده است</span></div></div><div className="spec-columns"><div><h3>مشخصات درخواست</h3><Spec label="رنگ‌ها" value="مشکی، سفید، سرمه‌ای"/><Spec label="سایزبندی" value="M تا 2XL · نسبت ۱:۲:۲:۱"/><Spec label="برند اختصاصی" value="لیبل گردن و بسته‌بندی اختصاصی"/><Spec label="تک‌پک" value="بله، Polybag مات"/></div><div><h3>فایل‌های پیوست</h3><File name="Tech-pack_oxford_v3.pdf" info="۲.۸ MB · PDF"/><File name="reference-front.jpg" info="۱.۲ MB · تصویر مرجع"/><File name="logo-helya.ai" info="۸۴۰ KB · لوگو"/></div></div><div className="quote-compare"><div><p>درخواست مشتری</p><b>۵۰۰ تکه · ۲۰ روز</b><span>Oxford Cotton 150gr</span></div><div className="arrow-circle">←</div><div className="supplier-offer"><p>پیشنهاد کارخانه</p><b>۶۰۰ تکه · ۲۸ روز</b><span>حداقل قابل تولید شما</span></div></div><div className="detail-actions"><button className="button secondary">رد درخواست</button><button className="button secondary">درخواست اطلاعات</button><button className="button primary" onClick={onQuote}><FileCheck2 size={16}/>ساخت پیشنهاد قیمت</button></div></section> }
