...
import { MediaDropField } from "../components/visualBuilder";
import { recommendStyleProducts } from "../lib/styleIntelligence";
import { useCatalogTaxonomy } from "../lib/catalogTaxonomy";
import ProductMediaStudio from "./ProductMediaStudio";
...
const tabs=[["basic","اطلاعات پایه"],["variants","تنوع و موجودی"],["mediaStudio","رسانه محصول"],["attributes","مشخصات و اندازه"],["styling","با این ست کنید"],["workflow","انتشار و نسخه‌ها"]] as const;
...
export default function AdminProductEditor({ initial, onBack, onSave }: { initial:AdminProductRecord; onBack:()=>void; onSave:(record:AdminProductRecord)=>string|null }) {
  ...
  return <div className="pb-20"><header data-testid="product-editor-toolbar" className="-mx-4 mb-5 border-y border-neutral-200 bg-white px-4 py-3 lg:-mx-6 lg:px-6"><div className="flex flex-wrap items-center gap-3"><button onClick={onBack} className={secondary}>بازگشت</button><div className="min-w-0"><h1 className="truncate text-[16px] font-medium">{draft.name||"محصول بدون نام"}</h1><p className="mt-0.5 text-[9px] text-neutral-400">{draft.specs.code} · {statusLabel[draft.admin.status]}</p></div><div className="mr-auto flex flex-wrap gap-2">{draft.admin.status==="published"&&<button onClick={()=>window.open(`${window.location.origin}${window.location.pathname}#/product/${draft.id}`,"_blank")} className={secondary}>پیش‌نمایش</button>}<button onClick={()=>save()} className={secondary}>ذخیره پیش‌نویس</button><button onClick={()=>save("review")} className={secondary}>ارسال برای بررسی</button><button onClick={()=>save("published")} className="h-9 bg-[#011c3a] px-4 text-[10px] text-white">انتشار محصول</button></div></div>{message&&<p role={message.type==="error"?"alert":"status"} className={(message.type==="error"?"border-red-200 bg-red-50 text-red-700":"border-[#b9cfbc] bg-[#edf3ee] text-[#36563a]")+" mt-3 border px-3 py-2 text-[10px]"}>{message.text}</p>}</header>
    ...
    <main className="border border-neutral-200 bg-white p-4 lg:p-6">
      {tab==="basic"&&<section className="grid gap-5 xl:grid-cols-[1fr_300px]"><div className="grid gap-3 sm:grid-cols-2"><Field label="نام محصول" required><input aria-label="نام محصول" value={draft.name} onChange={e=>update("name",e.target.value)} className={input}/></Field><Field label="نام لاتین"><input aria-label="نام لاتین" value={draft.latin} onChange={e=>update("latin",e.target.value)} className={input}/></Field><Field label="زیرعنوان"><input aria-label="زیرعنوان" value={draft.subtitle} onChange={e=>update("subtitle",e.target.value)} className={input}/></Field><Field label="کد محصول" required><input aria-label="کد محصول" value={draft.specs.code} onChange={e=>update("specs",{...draft.specs,code:e.target.value})} className={input}/></Field><Field label="قیمت پایه (تومان)" required><input aria-label="قیمت پایه (تومان)" type="number" min="0" value={draft.price} onChange={e=>update("price",Number(e.target.value))} className={input}/></Field><Field label="دسته‌بندی"><select aria-label="دسته‌بندی" value={draft.category} onChange={e=>{const category=taxonomy.categories.find(x=>x.id===e.target.value);setDraft(c=>({...c,category:e.target.value,categoryLabel:category?.label??c.categoryLabel}))}} className={input}>{taxonomy.categories.map(x=><option key={x.id} value={x.id}>{x.label}</option>)}</select></Field><Field label="استایل"><select aria-label="استایل" value={draft.style} onChange={e=>update("style",e.target.value)} className={input}>{taxonomy.styles.map(x=><option key={x.id} value={x.id}>{x.label}</option>)}</select></Field><Field label="فصل"><select aria-label="فصل" value={draft.season} onChange={e=>update("season",e.target.value)} className={input}>{!taxonomy.seasons.some(x=>x.label===draft.season)&&draft.season?<option>{draft.season}</option>:null}{taxonomy.seasons.map(x=><option key={x.id} value={x.label}>{x.label}</option>)}</select></Field><Field label="توضیحات" wide><textarea aria-label="توضیحات" rows={6} value={draft.description} onChange={e=>update("description",e.target.value)} className="w-full border border-neutral-300 p-3 text-[11px] outline-none focus-visible:ring-2 focus-visible:ring-[#011c3a]"/></Field></div><aside className="space-y-5"><div className="border border-neutral-200 p-4"><Field label="افزودن از کالکشن‌های تعریف‌شده"><select aria-label="کالکشن تعریف‌شده" defaultValue="" onChange={e=>{if(e.target.value&&!draft.admin.collections.includes(e.target.value))updateAdmin("collections",[...draft.admin.collections,e.target.value]);e.target.value=""}} className={input}><option value="">انتخاب کنید</option>{taxonomy.collections.map(x=><option key={x.id} value={x.label}>{x.label}</option>)}</select></Field></div><TokenEditor label="کالکشن‌ها" value={collection} onChange={setCollection} items={draft.admin.collections} onAdd={()=>addToken("collections",collection,()=>setCollection(""))} onRemove={x=>updateAdmin("collections",draft.admin.collections.filter(y=>y!==x))}/><TokenEditor label="تگ‌ها" value={tag} onChange={setTag} items={draft.admin.tags} onAdd={()=>addToken("tags",tag,()=>setTag(""))} onRemove={x=>updateAdmin("tags",draft.admin.tags.filter(y=>y!==x))}/><div className="border border-neutral-200 p-4"><p className="text-[10px] font-medium">آمادگی انتشار</p><ul className="mt-3 space-y-2 text-[9.5px]">{[draft.name?"نام تکمیل است":"نام ناقص است",draft.images.length?`${draft.images.length} تصویر` : "تصویر ندارد",draft.admin.variants.length?`${draft.admin.variants.length} تنوع` : "تنوع ندارد"].map((x,i)=><li key={x} className={((i===0&&draft.name)||(i===1&&draft.images.length)||(i===2&&draft.admin.variants.length)?"text-[#36563a]":"text-red-700")}>{x}</li>)}</ul></div></aside></section>}
      {tab==="variants"&&(
        <VariantsSection
          draft={draft}
      ...
      {tab==="mediaStudio"&&<ProductMediaStudio value={draft} onChange={setDraft}/>}
      {tab==="attributes"&&<DynamicProductSpecifications value={draft} onChange={setDraft}/>}
      {tab==="styling"&&(
        <RelationsSection
          draft={draft}
...
function RelationsSection({
  ...
  const smartSuggestions = recommendStyleProducts(draft, complementary, {}, 6);
  ...
  return (
    <section className="space-y-6">
      {/* انتخابگرهای محصول */}
      <div className="border border-[#b9cfbc] bg-[#f4f8f4] p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-[12px] font-medium">پیشنهاد هوشمند «با این ست کنید»</h2><p className="mt-1 text-[9.5px] text-neutral-500">یک لیست واحد از مکمل‌ها براساس دسته، فصل، استایل و هارمونی رنگ.</p></div><button onClick={()=>{const ids=smartSuggestions.slice(0,4).map(item=>item.product.id);onComplementaryChange(ids);onRelatedChange(ids)}} className="h-9 bg-[#011c3a] px-3 text-[10px] text-white">اعمال ست پیشنهادی</button></div><div className="mt-3 flex gap-2 overflow-x-auto">{smartSuggestions.map(item=><div key={item.product.id} className="min-w-44 border border-neutral-200 bg-white p-2"><div className="flex gap-2"><img src={item.product.images[0]} alt={item.product.name} className="h-12 w-10 object-cover"/><div className="min-w-0"><p className="truncate text-[9.5px] font-medium">{item.product.name}</p><p className="mt-1 text-[8.5px] text-[#36563a]">تطابق {item.score.toLocaleString('fa-IR')}٪</p><p className="mt-1 truncate text-[8px] text-neutral-400">{item.reasons.join(' · ')}</p></div></div></div>)}</div></div>
      <div className="grid gap-6">
        ...
        <div>
          <div className="flex items-center justify-between gap-2">
            <div>
              <h2 className="text-[12px] font-medium">محصولات ست پیشنهادی</h2>
              <p className="mt-1 text-[9.5px] text-neutral-500">محصولاتی که هم در کارت‌های مکمل و هم روی عکس تعاملی همین ست استفاده می‌شوند.</p>
            </div>
