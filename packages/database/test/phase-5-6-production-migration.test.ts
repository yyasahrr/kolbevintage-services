...
describe("Phase 5.6 PostgreSQL migration and invariants", () => {
  ...
  it("enforces QC arithmetic and immutable append-only evidence rows", async () => {
    await withClient(DB, async (client) => {
      ...
      await expect(client.query(`INSERT INTO quality_inspection (id, job_id, lot_id, checklist_id, checklist_version, sample_size, accepted_units, defect_units, rework_units, rejected_units, created_by) VALUES ('p56_inspection_bad', 'p56_job', 'p56_lot', 'p56_checklist', 1, 10, 10, 1, 0, 0, 'p56_admin')`)).rejects.toThrow();
