...
describe("Phase 5.7 PostgreSQL migration and invariants", () => {
  ...
  it("creates the seven promotion tables (journal idx 43 at the 5.11-A head; 0038 adds the return aggregate, 0039 alters the refund engine in place, 0040/0041 add search/browse indexes only, 0042 adds review proof columns, 0043 extends the admin permission catalog with the retail actions)", async () => {
    ...
    expect(journal.entries[journal.entries.length - 1]).toMatchObject({
      idx: 43,
      tag: "0043_phase_5_11_retail_admin_operations",
    });
  });
