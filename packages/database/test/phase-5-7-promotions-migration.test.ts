...
describe("Phase 5.7 PostgreSQL migration and invariants", () => {
  ...
  it("creates the seven promotion tables (0046 adds return-filing replay columns only; 0045 adds the suspicious-flag table)", async () => {
    ...
    expect(journal.entries[journal.entries.length - 1]).toMatchObject({
      idx: 46,
      tag: "0046_phase_5_11_return_filing_idempotency",
    });
  });
