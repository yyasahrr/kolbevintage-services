...
describe("Phase 5.7 PostgreSQL migration and invariants", () => {
  ...
  it("creates the seven promotion tables (journal idx 45 at the 5.11-C head; 0038 adds the return aggregate, 0039 alters the refund engine in place, 0040/0041 add search/browse indexes only, 0042 adds review proof columns, 0043 adds staff queue indexes only, 0044 extends the permission CHECK only, 0045 adds the suspicious-flag table)", async () => {
    ...
    expect(journal.entries[journal.entries.length - 1]).toMatchObject({
      idx: 45,
      tag: "0045_phase_5_11_c_admin_security",
    });
  });
