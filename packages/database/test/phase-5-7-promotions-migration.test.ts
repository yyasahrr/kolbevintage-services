...
describe("Phase 5.7 PostgreSQL migration and invariants", () => {
  ...
  it("creates the seven promotion tables (journal idx 42 at the 5.10-C head; 0038 adds the return aggregate, 0039 alters the refund engine in place, 0040/0041 add search/browse indexes only, 0042 adds review proof columns)", async () => {
    ...
    expect(journal.entries[journal.entries.length - 1]).toMatchObject({
      idx: 42,
      tag: "0042_phase_5_10_c_review_proof",
    });
  });
