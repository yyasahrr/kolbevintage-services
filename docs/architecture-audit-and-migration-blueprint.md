...
|---|---|---|---|
| D1 | **Retail checkout 500** (`jsonb` array param) — public sales broken | ✅ fixed 2026-09-17 | Phase 0; regression test `retail-checkout.spec.ts` |
| D2 | **VIP self-approval** on `wholesale/apply` (auto role elevation) | ✅ fixed 2026-09-17 | Phase 0; now `pending` → admin `accounts/:id/status`; UI wording + approve button added |
| D3 | Retail totals & lines from client, stored unvalidated | ✅ partial 2026-09-17 | Phase 0: server recomputes price/shipping/total from `retail_product`; no payment/inventory link yet (Phase 4/5); price list is still the hardcoded mirror until Phase 3 makes it the catalog's origin |
| D4 | No auth hardening: default HMAC secret fallback, LS tokens, no revocation/2FA/rate-limit | 🟠 partial | Phase 0: prod secret guard, HttpOnly cookie dual-write, logout, prod write-CORS closed. Remaining: clients still use LS Bearer; deny-list/2FA/rate-limit in Phase 2 (Nest `auth`) |
...
| D13 | No idempotency on order POSTs (double-click = double order) | ✅ fixed 2026-09-17 (order endpoints) | `Idempotency-Key` on `retail/orders` + `wholesale/orders`; general middleware at Phase 2 |
...
|---|---|---|---|---|
| **0. Stop-the-bleed** (this repo, Next.js) ✅ **DONE 2026-09-17** | D1 fix + regression test; D2 approval gate (pending → admin approve, default 365-day expiry, `INVALID_STATUS` allow-list); D3 server-side recompute of retail totals/shipping via `retail_product` price list + line snapshot sanitization; D4-lite: HttpOnly `kolbe_session` cookie dual-write + `/store/kolbe/auth/logout` + boot-time secret guard (instrumentation) + production write-CORS closed; `me` open to any authenticated role; `Idempotency-Key` on retail & wholesale order POSTs; 27 Vitest integration tests (real PG) + GitHub Actions CI (typecheck → test → build) | 🟢 low | git revert | checkout creates order w/ server totals; VIP pending; CI green — **all met; `tsc` ✅, vitest 27/27 ✅, `next build` ✅** |
